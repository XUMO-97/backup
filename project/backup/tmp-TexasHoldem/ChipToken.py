...
def Main(operation, args):
    ...
    if operation == 'addIssue':
        if len(args) != 2:
            return False
        addNum = args[0]
        return addIssue(addNum)
    return False
...
def addIssue(account, addNum):
    if CheckWitness(account) == False or addNum <= 0:
        return False
    if account != OWNER:
        return False
    totalNum = Get(ctx, SUPPLY_KEY)
    acctBalance = balanceOf(account)
    Put(ctx, SUPPLY_KEY, totalNum + addNum)
    Put(ctx, concat(BALANCE_PREFIX, account), acctBalance + addNum)
    TransferEvent("", OWNER, addNum)
    return True
