...
const utils = {
  ...
  getChallengeBarrier: (listing, account, applyStageLen) => {
    return new Eth.BN(abi.soliditySHA3(['bytes32', 'address'], [listing, account])).mod(new Eth.BN(applyStageLen)).toNumber()
  },
  ...
  addToWhitelistWithData: async (domain, deposit, data, actor, registry) => {
    await utils.as(actor, registry.apply, domain, deposit, data);
    await utils.increaseTime(paramConfig.applyStageLength + 1);
    await utils.as(actor, registry.updateStatus, domain);
  },
