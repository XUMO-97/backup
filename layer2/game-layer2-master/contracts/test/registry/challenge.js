...
contract('Registry', (accounts) => {
  describe('Function: challenge', () => {
    ...
    let enableMassChallengePrevention = async () => {
      // Enable mass challenge prevention
      const receipt = await utils.as(
        proposer, parameterizer.proposeReparameterization,
        'massChallengePreventionEnabled', 1,
      )
    ...
    it('should successfully challenge an application with mass challenge prevention enabled', async () => {
      await enableMassChallengePrevention()

      const listing = utils.getListingHash('failure.net')

      // Challenge after our barrier has passed
      await utils.as(applicant, registry.apply, listing, paramConfig.minDeposit, '')
      const barrier = utils.getChallengeBarrier(listing, challenger, paramConfig.applyStageLength)
      await utils.increaseTime(barrier + 1)
      await utils.challengeAndGetPollID(listing, challenger, registry)
      await utils.increaseTime(paramConfig.commitStageLength + paramConfig.revealStageLength + 1)
      await registry.updateStatus(listing)

      // Check that the challenge succeeded
      const isWhitelisted = await registry.isWhitelisted.call(listing);
      assert.strictEqual(isWhitelisted, false, 'An application which should have failed succeeded');
    });
    
    it('should unsuccessfully challenge an application with mass challenge prevention enabled', async () => {
      await enableMassChallengePrevention()

      const listing = utils.getListingHash('failure.net')

      // Challenge before our barrier has passed
      await utils.as(applicant, registry.apply, listing, paramConfig.minDeposit, '')
      const barrier = utils.getChallengeBarrier(listing, challenger, paramConfig.applyStageLength)
      utils.challengeAndGetPollID(listing, challenger, registry).then(res => {
