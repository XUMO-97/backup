...
contract Parameterizer is Ownable {
    ...
    */
    function init(
        ...
        require(_owner != 0 && address(owner) == 0);
        ...
        // The operator is a special parameter that stores the SSB public key of
        // the operator, which is 32-byte long.
        //
        // In order to keep the API and implementation simple, we decide to use
        // the same propose/challenge mechanism for configuring the operator.
        //
        // As a result, we need to convert the operator to a number back and forth.
        //
        // Note that this is safe since an address is 160-bit long and a uint is
        // 256-bit long, so there's no information loss.
        //
        // To get the current operator as an address, use getOperator()
        set("operator", uint(_operator));
    }
    ...
    */
    function getOperator() public view returns (bytes32 operator) {
        return bytes32(params[keccak256("operator")]);
    }
