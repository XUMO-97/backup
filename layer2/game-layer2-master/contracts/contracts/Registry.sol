...
contract Registry {
    ...
    struct Listing {
        ...
        bytes32 data;
        uint applyStageLen;     // The length of the apply stage when this application
                                // was made.  Need to record a snapshot since the length
                                // may change.
    }
    ...
    */
    function apply(bytes32 _listingHash, uint _amount, bytes32 _data) external {
        ...
        // Sets apply stage length and end time
        listing.applyStageLen = parameterizer.get("applyStageLen");
        listing.applicationExpiry = block.timestamp.add(listing.applyStageLen);
        ...
        emit _Application(_listingHash, _amount, listing.applyStageLen, listing.applicationExpiry, _data, msg.sender);
    }
    ...
    */
    function challenge(bytes32 _listingHash, bytes32 _data) external returns (uint challengeID) {
        ...
        if (!listing.whitelisted) {
            require(now > (listing.applicationExpiry - listing.applyStageLen + barrier(_listingHash)));
        }
    ...
    */
    ...
    /**
    @dev                Returns whether the listing has been accepted into the
                        registry, and the data associated with this listing if
                        it does.
    @param _listingHash The listingHash whose data is being requested.
    */
    function getData(bytes32 _listingHash) view public returns (bool exists, bytes32 data) {
        if (isWhitelisted(_listingHash)) {
            return (true, listings[_listingHash].data);
        } else {
            return (false, "");
        }
    }
    ...
    */
    ...
    /**
    @dev                Compute the barrier for the caller for the given listing.  The barrier is the
                        amount of time that the caller needs to wait before they can challenge.
    @param _listingHash The listing hash that identifies the listing.
    */
    function barrier(bytes32 _listingHash) public view returns (uint256) {
        Listing storage listing = listings[_listingHash];
        return uint256(keccak256(abi.encodePacked(_listingHash, msg.sender))) % listing.applyStageLen;
    }
}
