...
luckyNumberContractAddress = "f1afb2d9b7722659c2b3e5eb36f35a7cb0393c02"  # without magnitude
...
class TestAsset(unittest.TestCase):
    ...
    def test_buySomePaper(self):
        ...
        admin_pwd1 = "111111"
        adminAcct1 = sdk.wallet_manager.get_account(admin_addr1, admin_pwd1)
        hash = self.test_buyPaper(adminAcct1, 5)
        print("buy some paper hash is ", hash)
        return True
    ...
    def test_endCurrentRound(self):
        ...
        # return True
        # when pre-execute, don't use 0x67
        AdminAcct = sdk.wallet_manager.get_account(admin_addr1, admin_pwd1)
        param_list = []
    ...
    def test_addMultiReferral(self):
        accountListToBeUsed = []
        accountFromIndex = 2
        accountEndIndex = 6
        line_num = 0
        ...
        toBeReferred1 = accountListToBeUsed[0].get_address().to_array()
        toBeReferred2 = accountListToBeUsed[1].get_address().to_array()
        referral1 = accountListToBeUsed[2].get_address().to_array()
        referral2 = accountListToBeUsed[3].get_address().to_array()
        print("1st pair", accountListToBeUsed[0].get_address_base58(), accountListToBeUsed[2].get_address_base58() )
        print("2nd pair", accountListToBeUsed[1].get_address_base58(), accountListToBeUsed[3].get_address_base58())
        param_list = []
        param_list.append("addMultiReferral".encode())
        param_list1 = []
        param_list2 = []
        param_list2.append(toBeReferred1)
        param_list2.append(referral1)
        param_list3 = []
        param_list3.append(toBeReferred2)
        param_list3.append(referral2)
        param_list1.append(param_list2)
        param_list1.append(param_list3)
        param_list.append(param_list1)

        print("***** addMultiReferral", param_list)
        ...
        # # print("gasLimit1 in fill paper is ", gaslimit, type(gaslimit))
        # tx = NeoVm.make_invoke_transaction(bytearray(contract_address), bytearray(params), b'', 20000, 500)
        # sdk.sign_transaction(tx, adminAcct1)
        # print("tx is ", tx)
        # nil, gaslimit = sdk.rpc.send_raw_transaction_pre_exec(tx)
        # print("nil and gaslimit are ", nil, gaslimit)
        #
        # return True
        ...
        gaslimit = gaslimit * 2
        unix_time_now = int(time.time())
        tx = Transaction(0, 0xd1, unix_time_now, 500, gaslimit, adminAcct1.get_address().to_array(), params, bytearray(), [], bytearray())
        sdk.sign_transaction(tx, adminAcct1)
        buf = tx.serialize()
        tx_data = buf.hex()
        print("tx is ", tx_data)
        print("len(tx) is ", len(tx_data))
        hash = sdk.rpc.send_raw_transaction(tx)

        print("********** addMultiReferral***** hash is \n ", hash)
        return hash
    ...
    def test_getPaperBalance(self):
        abi_function = AbiFunction("getPaperBalance", "", [{"name": "account", "type": ""}])
        userBase58Address1 = "AXR2wouPRUfS2fCZ6sUvwts2fskkzRkpYy"
        print(Address.b58decode(userBase58Address1).to_array())

        abi_function.set_params_value((Address.b58decode(userBase58Address1).to_array(),))
        res, nil = sdk.neo_vm().send_transaction(contract_address, adminAcct, adminAcct, 0, 0, abi_function, True)
        tmp = res
        if not tmp:
            tmp = "00"
        ...
        tmp.reverse()
        returnedInt = int(tmp.hex(), 16)
        print(userBase58Address1, " == ", returnedInt)
    ...
    def test_getDividendsBalance(self):
        account = adminAcct
        userBase58Address = "AXDeX8WtzgnNwqRercuLPWq5NfMEHgj4T7"
    ...
    def test_ProfitPerPaper(self):
        currentRound = self.test_getCurrentRound()
        param_list = []
        # when pre-execute, don't use 0x67
        abi_function = AbiFunction("getProfitPerPaper", "", param_list)
        res, nil = sdk.neo_vm().send_transaction(contract_address, adminAcct, adminAcct, 0, 0, abi_function, True)
        print("res getProfitPerPaper is ", res)
        print("test-getProfitPerPaper is : ", res, " in round ", currentRound)
        return True
